  ...
  public void action() {
    ...
    if (agentKnowledge.reachDestination()) {
      ...
      if (!isSentTerminationInform) {
        ...
        msg.addReceiver(new AID(agentKnowledge.getTankerAgentName(), AID.ISLOCALNAME));
      ...
      if (myAgent.receive(msgTemplate) == null) {
        block();
      }
      
      System.out.println(agent.getLocalName() +  " successfully informs it termination to the tanker agent " + agentKnowledge.getTankerAgentName());
